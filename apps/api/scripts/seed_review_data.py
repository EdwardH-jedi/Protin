...
async def _upsert_match(
    session: AsyncSession, *, user_a: UUID, user_b: UUID, sport: str
) -> tuple[Match, bool]:
...
async def _seed_chat_if_empty(
    ...
    *,
    ...
    existing = await session.execute(
        select(Message.id).where(Message.match_id == match_id).limit(1)
    )
...
async def seed() -> int:
    ...
    if not reviewer_email or not reviewer_password:
        print(
            "[seed_review] REVIEWER_EMAIL and REVIEWER_PASSWORD must be set "
            "in the environment. Refusing to run.",
            file=sys.stderr,
        )
        return 2
    ...
    try:
        async with Session() as session:
            ...
            reviewer_user, created = await _get_or_create_user(
                session, email=reviewer_email, hashed_password=hashed
            )
            ...
            await _upsert_sport_profiles(
                session, user_id=reviewer_user.id, sport_seeds=REVIEWER_SPORTS
            )
            await _ensure_identity_preferences(session, user_id=reviewer_user.id)
            print(
                f"[seed_review] reviewer {'created' if created else 'updated'}"
            ...
            for demo in _DEMO_USERS:
                u, was_created = await _get_or_create_user(
                    session, email=demo.email, hashed_password=None
                )
                ...
                await _upsert_sport_profiles(
                    session, user_id=u.id, sport_seeds=demo.sport_profiles
                )
            ...
            for demo_email, sport in _MUTUAL_MATCH_EMAILS:
                ...
                print(
                    f"[seed_review] match {'created' if was_created else 'existing'}"
                    f" sport={sport} partner={demo_email}"
                )
            ...
            for demo_email, msgs in _CHAT_SEED.items():
                ...
                print(
                    f"[seed_review] chat  {action} match={m.id}"
                    f" partner={demo_email} inserted={inserted}"
                )
            ...
            for bk_seed in _BOOKING_SEEDS:
                ...
                if m is None:
                    print(
                        f"[seed_review] WARN no match for booking partner "
                        f"{bk_seed.partner_email} — skipping booking",
                        file=sys.stderr,
                    )
                    continue
