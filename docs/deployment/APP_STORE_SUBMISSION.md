...
- Hosted URLs for the legal docs are **live** on Netlify at `https://sportgang.netlify.app/{privacy,terms,support}/`. The matching `EXPO_PUBLIC_PRIVACY_URL`, `EXPO_PUBLIC_TERMS_URL`, and `EXPO_PUBLIC_SUPPORT_URL` values are documented in the env example files (`apps/mobile/.env.example`, `apps/mobile/.env.staging.example`, `.env.example`). Still required: pin the same three values onto the EAS preview + production environments via `eas env:create` (commands in `docs/deployment/APPLE_TESTFLIGHT_PREP.md` §4.8).
...
- [ ] `EXPO_PUBLIC_PRIVACY_URL`, `EXPO_PUBLIC_TERMS_URL`, `EXPO_PUBLIC_SUPPORT_URL` set on EAS preview + production profiles to the Netlify URLs above (see `docs/deployment/APPLE_TESTFLIGHT_PREP.md` §4.8)
