...
Split between what the repo already supports today (no engineering work needed) and what still has to happen before you can submit. Every "still required" item is expanded in §11 or in its relevant section below.
...
- `apps/api/scripts/seed_review_data.py` does not exist. The review test account + two seed accounts + a pending booking (see §6) depend on it.
...
- Screenshots at both required iPhone sizes (§9), 6 per size.
- Apple Developer Program enrolment + ASC account (§Prerequisites).
- TestFlight internal test with at least 2 human testers.

Subscriptions / in-app purchases are **not** implemented. Do not select any IAP options in App Store Connect; the privacy answers in §4 must continue to reflect this.
...
| Age Rating | See §5 below — answer the questionnaire to generate |
...
Source of truth: `docs/legal/PRIVACY_POLICY.md` §2. Apple's privacy questionnaire maps to these answers:
...
| Crash Data (Sentry) | Yes¹ | No | No | App Functionality, Analytics |
| Performance Data (Sentry) | Yes¹ | No | No | App Functionality, Analytics |
...
¹ Sentry only initialises when `EXPO_PUBLIC_SENTRY_DSN` is set at build time (`apps/mobile/App.tsx`). If you ship without a DSN, switch the two Sentry rows to **No** in the App Privacy questionnaire — collecting nothing is fine; mis-declaring is what triggers a 5.1.2 rejection.
...
| Notes | See §7 "Review notes" below — paste the full block |
...
|---|---|---|
| iPhone 6.9" (Pro Max, 16 Pro Max) | 1290 × 2796 px, portrait | 3–10 |
| iPhone 6.5" (Plus / Pro Max pre-16) | 1242 × 2688 px, portrait | 3–10 |
...
- `File → Save Screen` (⌘S) while the app is in foreground.
...
- [ ] `apps/mobile/assets/` created and populated. `notification-icon.png` is hard-required by `app.config.js`; app icon (1024×1024 for iOS), splash image, and Android adaptive-icon foreground need to be added and wired into `app.config.js` for App Store visual review to pass.
- [ ] `EXPO_PUBLIC_SENTRY_DSN` set for the production `eas build` profile **iff** you want crash reporting at launch. If unset, downgrade the two Sentry rows in §4 to "No".
...
- [ ] Review test account seeded, credentials captured for §6 above
...
| 5.1.2 — mismatched privacy answers vs actual data collection | Re-check §4 against the privacy policy. |
